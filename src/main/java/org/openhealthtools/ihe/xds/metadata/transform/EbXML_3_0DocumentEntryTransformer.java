 ...
 *
 */
public class EbXML_3_0DocumentEntryTransformer implements DocumentEntryTransformer {
	
	/** Descriptor for Transformer */
	public static final String DESCRIPTOR = "EBXML_3_0_DE_XFRMR";
	
	/** Logger */
	private static final Logger logger = Logger.getLogger(EbXML_3_0DocumentEntryTransformer.class);
	 ...
	 */
	public EbXML_3_0DocumentEntryTransformer() {
		this.docData = null;
	...
	public void transform(DocumentEntryType docEntry) throws MetadataTransformationException{
		...
		if(docEntry.getParentDocument() != null){
			...
			this.parentData.setId(EbXML_3_0TransformerUtils.getRandomId("a", 0));
			...
			if(docEntry.getParentDocument().getParentDocumentRelationship() != null){
				this.parentData.setAssociationType(EbXML_3_0TransformerConstants.IHE_ASSOCIATION_TYPE_PREFIX + docEntry.getParentDocument().getParentDocumentRelationship().getName());
			}
	...
	private void setExtrinsicObjectAttributes(DocumentEntryType docEntry) {
		...
		if (docEntry.isSetAvailabilityStatus()) {
			if (docEntry.getAvailabilityStatus().equals(AvailabilityStatusType.APPROVED_LITERAL)) {
				...
				docData.setStatus(EbXML_3_0TransformerConstants.WITHDRAWN_STATUS);
			}
		}
		// set mimeType
	...
	private void setExtrinsicObjectClassifications(DocumentEntryType docEntry) {
		...
		if(docEntry.getClassCode() != null){
			...
			c.setId(EbXML_3_0TransformerUtils.getRandomId("c", count));
			count++;
